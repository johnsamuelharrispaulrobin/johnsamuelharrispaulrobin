...
batch and incremental, from raw ingestion to analytics-ready layers.

Currently finishing my M.Tech in Data Science & AI, with research focused
on ML-driven adaptive learning systems — XGBoost recall prediction, MPC
scheduling, and sentence transformer embeddings.

**Actively looking for Data Engineer / Data Platform roles in Germany and Europe.**
...
### 🔧 [de-training-camp](https://github.com/johnsamuelharrispaulrobin/de-training-camp)
dbt project covering dimensional modelling, incremental models, custom macros,
schema tests, and seed-based reference data — built to sharpen production-grade
dbt skills using DuckDB as the local warehouse engine.

`dbt` · `DuckDB` · `SQL` · `data modelling`

---

### 🧠 [adaptive-learning-rebuild](https://github.com/johnsamuelharrispaulrobin/adaptive-learning-rebuild)
ML pipeline for personalised spaced repetition scheduling. Uses XGBoost to
predict recall probability, Model Predictive Control to optimise review timing,
and sentence transformers to model semantic similarity between concepts.
Grounded in research for my M.Tech dissertation.

`Python` · `XGBoost` · `MPC` · `sentence-transformers` · `ML pipelines`
